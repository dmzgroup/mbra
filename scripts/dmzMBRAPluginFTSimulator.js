var dmz =
   ...
   , VulnerabilityReducedHandle = dmz.defs.createNamedHandle ("FT_Vulnerability_Reduced_Value")
...
function start_work () {
   ...
   if (root) {
      risk = 0;
      var vulnerability = traverse_fault_tree (root);
      if (!vulnerability) { vulnerability = 0; }
   ...
   if (not_zero (budget)) {
      ...
      var B = 0;
      Object.keys (index).forEach (function (key) {
         if (index[key].vulnerability <= 0) { index[key].vulnerability = 1; }
         index[key].gamma = -Math.log (vinf / index[key].vulnerability);
         if (index[key].gamma < 0) { index[key].gamma = 0; }
      ...
      Object.keys (index).forEach (function (key) {
         var C = 0;
         if (not_zero (index[key].gamma)) { C = index[key].cost / index[key].gamma; }
         var D = log_defender (index[key]);
         index[key].allocation = -C * (logLamda + D);
         if (index[key].allocation < 0) { index[key].allocation = 0; }
      ...
      if (totalAllocation < budget) {
         ...
         for (var count = 0; count < index.length && not_zero (remainder); count += 1) {
            var object = index[count];
   ...
   if (!haveSetTimer) {
      ...
      work = dmz.time.setRepeatingTimer (self, function () {
         ...
         if (size > 1) {
            var source = dmz.util.randomInt (0, size - 1);
            var target = source;
            var count = 0;
            ...
            if (source && target && source.handle != target.handle) {
               var sourceAllocation = source.allocation;
               var targetAllocation = target.allocation;
               var maxAddition = target.cost - target.allocation;
               if (maxAddition > source.allocation) { maxAddition = source.allocation; }
               if (maxAddition >= 0.01) {
                  var addition = 0.01 + ((maxAddition - 0.01) * Math.random ());
                  dmz.object.scalar (
                        source.handle,
                        AllocationHandle,
                  ...
                  var riskOrig = dmz.object.scalar (root, RiskSumReducedHandle);
                  var vulnerabilityOrig = dmz.object.scalar (root,
...
function get_logic_state (node) {
   ...
   var logic = OrState;
   if (logicTable) {
      var logicState = dmz.object.state (logicTable[1]);
      if (logicState) {
         if (logicState.contains (AndMask)) { logic = AndState; }
         else if (logicState.contains (XOrMask)) { logic = XOrState; }
      }
   }
   return logic;
};
...
dmz.object.create.observe (self, function (objHandle, objType) {
	if (objType.isOfType (ThreatType)) {
		...
		reset = true;
	} else if (objType.isOfType (ComponentType)) { reset = true; }
});

dmz.object.destroy.observe (self, function (objHandle) {
   if (objects[objHandle]) { delete objects[objHandle]; reset = true; }
   else if (dmz.object.type(objHandle).isOfType (ComponentType)) { reset = true;}
   if (objHandle == root) { root = null; }
});

dmz.object.state.observe (self, function (objHandle, attrHandle, value, prevValue) {
	if (value && prevValue) {
		if (!(value.and (LogicMask)).equal(prevValue.and (LogicMask))) { reset = true; }
	}
});

dmz.object.link.observe (self, FTLinkHandle, function (link, attr, Super, sub) {
	if (objects[Super] || objects[sub]) { reset = true; }
});
...
dmz.object.flag.observe (self, FTActiveFaultTree, function (object, attrHandle, value) {
	if (value) { root = object; reset = true; }
});

dmz.object.scalar.observe (self, EliminationHandle, function (objHandle, attrHandle, value) {
...
dmz.object.scalar.observe (self, ConsequenceHandle, function (objHandle, attrHandle, value) {
	if (objects[objHandle]) {
...
dmz.object.scalar.observe (self, VulnerabilityHandle, function (objHandle, attrHandle, value) {
	if (objects[objHandle]) {
...
dmz.object.scalar.observe (self, AllocationHandle, function (objHandle, attrHandle, value) {
...
function build_index (node) {
   var nodeList = null;
   if (node) { nodeList = dmz.object.subLinks (node, FTLinkHandle); }
   if (nodeList) {
      var keys = Object.keys (nodeList);
      keys.forEach (function (key) {
         var otype = dmz.object.type (nodeList[key]);
         if (otype.isOfType (ThreatType)) {
            var ref = objects[nodeList[key]];
            if (ref) { index.push (ref); }
         } else if (otype.isOfType (ComponentType)) { build_index (nodeList[key]); }
      });
   }
};
...
function risk_sub (objects) {
   ...
   keys.forEach (function (key) {
      result += (objects[key].threat * objects[key].vreduced * objects[key].consequence);
   });
   return result;
};
...
function risk_xor (objects) {
   var result = 0;
   var ikeys = Object.keys (objects);
   ikeys.forEach (function (ikey) {
      var value = 1;
      var current = objects[ikey];
      var jkeys = Object.keys (objects);
      jkeys.forEach (function (jkey) {
         if (ikey != jkey) {
            value *= (1 - (objects[jkey].threat * objects[jkey].vreduced));
         } else { value *= objects[jkey].vreduced; }
      });
...
function vulnerability_and (subv) {
   var result = 1;
   Object.keys (subv).forEach (function (key) { result *= subv[key].vt; });
   return result;
};

function vulnerability_or (subv) {
   var result = 0;
   if (subv.length == 1) { result = subv[0].vt; }
   else if (subv.length > 1) {
      result = 1;
      Object.keys (subv).forEach (function (key) { result *= (1 - subv[key].vt); });
      result = 1 - result;
   }
   return result;
};
...
function vulnerability_xor (subv) {
   ...
   Object.keys (subv).forEach (function (ikey) {
      ...
      Object.keys (subv).forEach (function (jkey) {
         if (jkey != ikey) { product *= (1 - subv[jkey].vt); }
         else { product *= subv[jkey].vreduced; }
      });
      result += product;
   });
   return result;
...
var traverse_fault_tree = function (node) {
   var result = null;
   var nodeList = dmz.object.subLinks (node, FTLinkHandle);
   ...
   if (nodeList.length > 0) {
      ...
      Object.keys (nodeList).forEach (function (key) {
         ...
         if (otype.isOfType (ThreatType)) {
            ...
            var value = traverse_fault_tree (nodeList[key]);
            if (value) { subv.push ({ vt: value, vreduced: value }); }
         }
      });
      var op = get_logic_state (node);
      result = 0;
      switch (op) {
         case AndState:
...
function log_defender (object) {
   var result = object.threat * object.consequence * object.vulnerability *
      object.gamma;
   if (result > 0) { result = Math.log (object.cost / result); }
   else { result = 0; }
   return result;
};
...
simMessage.subscribe (self, function (data) {
   if (dmz.data.isTypeOf (data)) {
      if (data.boolean ("Boolean", 0)) { start_work (); }
      else { stop_work (); }
   }
});
...
function stop_plugin () { dmz.time.cancleTimer (self, work); haveSetTimer = false; };
