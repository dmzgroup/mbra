...
var update_degrees = function (handle) {
   var count = 0;
   var sub = dmz.object.subLinks (handle, LinkHandle);
   if (sub) { count += sub.length; }
   var superLinks = dmz.object.superLinks (handle, LinkHandle);
   if (superLinks) { count += superLinks.length; }
   dmz.object.scalar (handle, DegreeHandle, count);
};

dmz.object.create.observe(self, function (handle, objType, varity) {
   if (objType) {
      if (objType.isOfType (NodeType)) { list[handle] = true; }
...
dmz.object.destroy.observe(self, function (handle) { delete list[handle]; });

dmz.object.link.observe (self, LinkHandle, function (link, attr, superLink, sub) {
   if (list[superLink]) { update_degrees (superLink); }
   if (list[sub]) { update_degrees (sub); }
});
