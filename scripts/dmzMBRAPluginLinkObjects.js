...
(function () {
   if (firstHandle) { dmz.object.activate(firstHandle); }
   if (secondHandle) { dmz.object.activate(secondHandle); }
})();
...
startMessage.subscribe (self, function (data) {
   ...
   if (dmz.data.isTypeOf (data)) {
      ...
      if (Handle && firstHandle && secondHandle) {
         var pos = dmz.object.position (Handle);
         if (pos) {
...
endMessage.subscribe (self, function (data) {
   ...
   if (dmz.data.isTypeOf (data)) {
      var handle = data.handle ("object", 0);
      if (handle) {
         if (handle == Handle) { Handle = null; handle = null; }
         else if (dmz.object.isObject (handle) &&
               dmz.object.type (handle).isOfType (NodeType) && Handle) {
            var undoHandle = dmz.undo.startRecord (undoLinkRecord);
            var linkHandle = dmz.object.link (NodeLink, Handle, handle);
            var attrObj = null;
            if (linkHandle) {
               ...
               var nalpHandle = dmz.defs.createNamedHandle(
                     "NetworkAnalysisLinkProperties");
...
//   self.obs.register (self.endMessage, receive_}, self);
